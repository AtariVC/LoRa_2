        ...
        if(pkt->target_id == driver->self_addr || pkt->target_id  == 0xFF){
            ...
            if(pkt->arg_len == driver->rx_pkt_len - 5){
                ...
                case SET_TIME:  // hour, min, sec
                    memcpy((uint8_t*)(&rtc_time_reg), pkt->args, 3);
                    RTC_set_time(RTC_struct_brief_time_converter(rtc_time_reg));
                    break;
                case SET_DATE:  // year, weak_day, month, day
                    memcpy((uint8_t*)(&rtc_date_reg), pkt->args, 4);
                    RTC_set_date(RTC_struct_brief_date_converter(rtc_date_reg));
                    break;
