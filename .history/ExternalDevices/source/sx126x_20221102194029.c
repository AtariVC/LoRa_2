...
void SX126x_Init(SX126x *driver){
    ...
    SX126x_SetDioIrqParams(driver, 0x3F, 1, 0, 0);
...
void SX126x_GetIrqStatus(SX126x *driver){
    ...
    SX126x_CalculateMode(driver, buf[0]);
    uint32_t addr = &(driver->irq_status);
    uint32_t addr2 = &(driver->irq_status.RxDone);
    uint16_t irq_status = *(uint16_t *)({buf[2], buf[1]});
    for(uint8_t i = 0; i < 8; i++){
        if((buf[2] >> i) & 0x01)
            *((uint8_t *)(&(driver->irq_status)) + i) = 1;
    }
    for(uint8_t i = 0; i < 2; i++){
        if((buf[1] >> i) & 0x01)
            *((uint8_t *)(&(driver->irq_status)) + i + 8) = 1;
    }
}
