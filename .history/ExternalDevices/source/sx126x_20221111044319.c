            ...
            if(pkt->arg_len == driver->rx_pkt_len - 5){
                ...
                case MY_PERIODIC_DATA:
                    ...
                    if(pkt->sender_id == 2) {
                        ...
                        (*buffer_counter) = gs2_buf_ptr;
                    } else if(pkt->sender_id == 3) {
                        buffer_ptr = gs3_buffer;
                        (*buffer_counter) = gs3_buf_ptr;
                    }
                    for(uint8_t i = 0; i < pkt->arg_len; i++){
                        memcpy(buffer_ptr + (*buffer_counter), &current_rtc, pkt->arg_len);
                        memcpy(buffer_ptr + (*buffer_counter) + sizeof(current_rtc), pkt->args, pkt->arg_len);
                        (*buffer_counter) += pkt->arg_len;
                    }
