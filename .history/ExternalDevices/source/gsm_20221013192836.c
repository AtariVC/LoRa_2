...
void GSM_RX_Handler(){
    ...
    if(sim7000g.uart->ISR & USART_ISR_IDLE){
        ...
        // }
            ...
            char trash[15];
            sscanf(sim7000g.rx_buf, "%s %hhd%s", trash, &sim7000g.signal_level, trash);
        }
            ...
            uint8_t percent = 0;
            char trash[15];
            sscanf(sim7000g.rx_buf, "%s %hhd,%hhd,%hd%s", trash, &charger_status, &percent, &sim7000g.vbat, trash);
        }
        rx_counter = 0;
        memset(sim7000g.rx_buf, 0, GSM_RX_BUFFER_SIZE);
    }
