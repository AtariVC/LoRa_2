        ...
        if(pkt->target_id == driver->self_addr || pkt->target_id  == 0xFF){
            ...
            if(pkt->arg_len == driver->rx_pkt_len - 5){
                ...
                case ECHO:
                    ...
                    pkt->sender_id = buf;
                    SX126x_SendData(driver, driver->rx_data, driver->rx_pkt_len);
                    break;
                ...
                case GET_BATTERY:
                    ...
                    break;
                case MY_PERIODIC_DATA:
                    for(uint8_t i = 0; i < pkt->arg_len; i++){
                        if(pkt->sender_id == 2) {
                            if(pkt->arg_len + gs2_buf_ptr + sizeof(current_rtc) < sizeof(gs2_buffer)){
                                RTC_get_time(&current_rtc);
                                memcpy(gs2_buffer + gs2_buf_ptr, &current_rtc, pkt->arg_len);
                                memcpy(gs2_buffer + gs2_buf_ptr + sizeof(current_rtc), pkt->args, pkt->arg_len);
                                gs2_buf_ptr += pkt->arg_len;
                            }
                        }
                        else if(pkt->sender_id == 3) {
                            if(pkt->arg_len + gs3_buf_ptr + sizeof(current_rtc) < sizeof(gs3_buffer)){
                                RTC_get_time(&current_rtc);
                                memcpy(gs3_buffer + gs3_buf_ptr, &current_rtc, pkt->arg_len);
                                memcpy(gs3_buffer + gs3_buf_ptr + sizeof(current_rtc), pkt->args, pkt->arg_len);
                                gs3_buf_ptr += pkt->arg_len;
                            }
                        }
                    }
