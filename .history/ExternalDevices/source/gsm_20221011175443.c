...
void GSM_SendSMS(GSM *driver, char *data, char *phone_num){
    ...
    GSM_wait_for_answer(driver, 1000);
    char *cmd = "AT+CMGS=\"";
    size_t cmd_len = strlen(cmd);
    size_t phone_len = strlen(phone_num);
    size_t data_len = strlen(data);
    char *buf = malloc(data_len + phone_len + cmd_len + 3);  // "AT+CMGS=\"+7xxxxxxxxxx\"\r"
    memcpy(buf, cmd, cmd_len);
    memcpy(buf + cmd_len, phone_num, phone_len);
    memcpy(buf + cmd_len + phone_len, "\"\r", 2);
    memcpy(buf + cmd_len + phone_len + 2, data, data_len);
    memcpy(buf + cmd_len + phone_len + 2 + data_len, (char*)({0x1A, 0x00}), 2);
    GSM_SendCMD(driver, buf);
    free(buf);
}
