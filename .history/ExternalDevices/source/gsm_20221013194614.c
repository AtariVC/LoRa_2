...
void GSM_RX_Handler(){
    ...
    if(sim7000g.uart->ISR & USART_ISR_IDLE){
        ...
        // }
            ...
            char trash[64];
            sscanf(sim7000g.rx_buf, "%s %d%s", trash, &sim7000g.signal_level, trash);
        }
            ...
            uint8_t percent = 0;
            char trash[64] = {0};
            uint8_t var = sscanf(sim7000g.rx_buf, "AT+CBC\r\r\n+CBC: %hhd,%hhd,%hd%s", trash, &charger_status, &percent, &sim7000g.vbat, trash);
            var++;
        }
        rx_counter = 0;
        memset(sim7000g.rx_buf, 0, GSM_RX_BUFFER_SIZE);
