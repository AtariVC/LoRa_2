...
void GSM_SetAPN(GSM *driver){
    char *cmd = "AT+CSTT=";
    size_t len = strlen(driver->APN);
    char *buf = malloc(strlen(cmd) + strlen(driver->APN));
    memcpy(buf, cmd, strlen(cmd));
    memcpy(buf + strlen(cmd), driver->APN, strlen(driver->APN));
    GSM_SendCMD(driver, driver->APN);
}

void GSM_CheckSignal(GSM *driver){
    GSM_SendCMD(driver, "AT+CSQ?\r");
}
uint16_t GSM_GetVBAT(GSM *driver){
    GSM_SendCMD(driver, "AT+CBC\r");
    return driver->vbat;
}
...
void GSM_OpenConnection(GSM *driver, char *ip, char *port){
    if(driver->ip_status == GPRS_STATUS){
        ...
        // char *buf1 = malloc()
        // char *buf =
        GSM_SendCMD(driver, "AT+CIPSTART=\"TCP\",\"http://hirundo.ru\",80\r");
    }
}
...
void GSM_SetDNS(GSM *driver){
    GSM_SendCMD(driver, "AT+CDNSCFG=\"8.8.8.8\",\"8.8.4.4\"\r");
}
...
void GSM_CheckIPstatus(GSM *driver){
    GSM_SendCMD(driver, "AT+CIPSTATUS\r");
}

void GSM_ActivateContext(GSM *driver){
    GSM_SendCMD(driver, "AT+CIICR\r");
}
void GSM_RequestIP(GSM *driver){
    GSM_SendCMD(driver, "AT+CIFSR\r");
}
void GSM_RX_Handler(){
