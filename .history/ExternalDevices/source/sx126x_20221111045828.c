        ...
        if(pkt->target_id == driver->self_addr || pkt->target_id  == 0xFF){
            ...
            if(pkt->arg_len == driver->rx_pkt_len - 5){
                ...
                case MY_PERIODIC_DATA:
                    ...
                    if((*buffer_counter_ptr) + sizeof(current_rtc) + pkt->arg_len < sizeof(data_buffer)){
                        for(uint8_t i = 0; i < pkt->arg_len; i++){
                            memcpy(gs_buffer_ptr + (*buffer_counter_ptr), &current_rtc, pkt->arg_len);
                            memcpy(gs_buffer_ptr + (*buffer_counter_ptr) + sizeof(current_rtc), pkt->args, pkt->arg_len);
                            (*buffer_counter_ptr) += pkt->arg_len + sizeof(current_rtc);
                        }
                    }
