...
uint8_t GSM_Init(GSM *driver){
    ...
    if(sim7000g.frame_error_counter > 0)
        GSM_SetPowerState(&sim7000g, HIGH);
    Delay(10000);
...
void GSM_SendTCP(GSM *driver, char *data, uint8_t data_len){
    UART_tx_string(driver->uart, "AT+CIPSEND");
    UART_tx(driver->uart, '\r');
    Delay(500);
    for(uint8_t i = 0; i < data_len; i++) UART_tx(driver->uart, data[i]);
...
void GSM_AnswerParser(){
    ...
    else if(strstr(sim7000g.rx_buf, "cmd") != 0){
        sim7000g.status.last_answer = 0;
        gpio_toggle(LED);
    }
    ...
    // }
        ...
        sim7000g.vbat = (uint16_t)sim7000g.vbat;
    }
