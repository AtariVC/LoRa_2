...
void SX126x_SendData(SX126x *driver, uint8_t *data, uint8_t data_len){
    ...
    SX126x_SetPacketParams(driver, driver->preamble_len, driver->header_type, data_len-1, driver->crc_on_off, driver->iq_polarity);
...
void SX126x_RxDataParse(SX126x *driver){
    if(driver->rx_pkt_len > 3){
        CommandStruct *pkt = (CommandStruct *)(driver->rx_data);
        if(driver->rx_data[2] == driver->self_addr || driver->rx_data[2] == 0xFF){
            if(driver->rx_data[4] == driver->rx_pkt_len - 4){
                switch (driver->rx_data[3])
                {
