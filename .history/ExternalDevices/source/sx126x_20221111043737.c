        ...
        if(pkt->target_id == driver->self_addr || pkt->target_id  == 0xFF){
            ...
            if(pkt->arg_len == driver->rx_pkt_len - 5){
                ...
                case MY_PERIODIC_DATA:
                    ...
                    uint32_t *buffer_ptr;
                    uint32_t *buffer_counter_ptr;
                    if(pkt->sender_id == 2) {
                        buffer_ptr = gs2_buffer;
                        buffer_counter_ptr = gs2_buf_ptr;
                    }
                    else if(pkt->sender_id == 3) {
                        buffer_ptr = gs3_buffer;
                        buffer_counter_ptr = gs3_buf_ptr;
                    }
                    for(uint8_t i = 0; i < pkt->arg_len; i++){
                        memcpy(buffer_ptr + (*buffer_counter_ptr), &current_rtc, pkt->arg_len);
                        memcpy(buffer_ptr + (*buffer_counter_ptr) + sizeof(current_rtc), pkt->args, pkt->arg_len);
                    }
