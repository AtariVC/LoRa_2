...
uint8_t check_flag(uint32_t flag, const uint8_t timeout_ms){
    uint8_t counter = timeout_ms;
    while((counter--) && flag) Delay(1);
    if(counter == 0) return 1;
    return 0;
}

uint8_t OneWire_SendBit(OneWire *ow, uint8_t data){
	check_flag(!(ow->uart->ISR & USART_ISR_TC), 100);
	ow->uart->TDR = (data == 1) ? OneWire_bit_1 : data;
	check_flag(!(ow->uart->ISR & USART_ISR_TC), 100);
	if(check_flag(!(ow->uart->ISR & USART_ISR_RXNE), 100)) ow->empty_bus = 1;
...
OneWireStatus OneWire_Reset(OneWire *ow){
    ...
    if(answer == OneWire_reset_cmd || ow->empty_bus) return ONE_WIRE_EMPTY_BUS;
    return ONE_WIRE_OK;
}
