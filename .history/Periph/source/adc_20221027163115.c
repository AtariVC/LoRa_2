...
void ADC_InitChannel(ADC *ADC_struct, uint8_t ch_num, GPIO_Pin gpio, ADC_CH_Group group, ADC_CH_SMP_Time smp_time){
    ...
    if(group == ADC_CH_Regular){
        ...
        ADC_struct->ADCx->SQR1 |= ch_num << (((ch_num - 1) * 6) + 6);
        if(ch_num <= 4)
            ADC_struct->ADCx->SQR1 |= ch_num << (ch_num * 6);
        else if(ch_num <= 9)
            ADC_struct->ADCx->SQR2 |= ch_num << (ch_num * 6) - 30;
        else if(ch_num <= 14)
            ADC_struct->ADCx->SQR2 |= ch_num << (ch_num * 6) - 60;
    }
}
