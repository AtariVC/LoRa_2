...
void ADC_InitRegChannel(ADC *ADC_struct, uint8_t ch_num, GPIO_Pin gpio, ADC_CH_Group group, ADC_CH_SMP_Time smp_time){
    ...
    ADC_struct->reg_channel_queue[ADC_struct->reg_ch_amount].group = group;
...
void ADC_Handler(){
    ...
    if(adc.ADCx->ISR & ADC_ISR_EOC){  // Inside the regular sequence, after each conversion is complete
        ...
        adc.reg_channel_queue[adc.last_conv_reg_ch - 1].result = adc.ADCx->DR;
        adc.last_conv_reg_ch++;
    }
    if(adc.ADCx->ISR & ADC_ISR_EOS){  // After the regular sequence is complete
        adc.ADCx->ISR |= ADC_ISR_EOS;
        adc.reg_channel_queue[adc.last_conv_reg_ch - 1].result = adc.ADCx->DR;
        adc.last_conv_reg_ch = 1;
    }
