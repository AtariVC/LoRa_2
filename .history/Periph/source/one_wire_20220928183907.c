...
// 'data' argument should be OneWire_bit_1 or OneWire_bit_0
    ...
    while(bitNum < OneWire_ROM_ID_lenth){
        ...
        if (cB != cmp_cB) searchDirection = cB;
            ...
            if (searchDirection == 0)
                // запоминаем развилку, в которой поворачивали налево
                zeroFork = bitNum;
            // else повернули на развилке вправо
        }
        else return -1;  // empty bus or algorythm issue  cB == cmp_cB && cB == 1

        if (searchDirection)  // сохраняем бит
            *(current_byte_in_rom) |= bit_in_byte_pos;
