...
void OneWireInitStruct(OneWire *ow) {
...
int8_t hasNextRom(OneWire *ow, uint8_t *ROM) {
    ...
    OneWireWrite(ow, 0xFF);  // OneWire Search cmd

    uint8_t bitNum = 0;
    int8_t zeroFork = -1;
    while(bitNum < OneWire_ROM_ID_lenth){
        uint8_t byteNum = bitNum >> 3;
        uint8_t bit_pos_in_byte = (1 << (bitNum & 0x07));
        ...
        if (cB != cmp_cB) searchDirection = cB;
            ...
            if (bitNum == ow->last_diff_bit_position)
                ...
                searchDirection = (uint8_t) ((ow->lastROM[byteNum] & bit_pos_in_byte));
        ...
        if (searchDirection)
            *(current_byte_in_rom) |= bit_pos_in_byte; // сохраняем бит
...
int OneWireSearchCmd(OneWire *ow) {
    uint8_t device_counter = 0, nextROM;
    owInit(ow);
    while (nextROM && device_counter < MAXDEVICES_ON_THE_BUS){
        nextROM = hasNextRom(ow, &ow->ids[device_counter]);
        if (nextROM < 0)
            return -1;
        device_counter++;
    }
