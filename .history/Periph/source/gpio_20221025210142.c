...
void gpio_state(GPIO_Pin gpio, GPIO_State state){
    if(gpio == uninitialized) return;
    gpio--;
	uint8_t port_num = gpio / NUMBER_OF_PINS_ON_EACH_PORT;
	uint8_t pin = gpio % NUMBER_OF_PINS_ON_EACH_PORT;
    GPIO_TypeDef *gpio_port = (GPIO_TypeDef *)(AHB2PERIPH_BASE + (0x0400UL << port_num));
	gpio_port->BSRR = 0x01 << (pin + (!state) * 16);
}
...
GPIO_State gpio_read(GPIO_Pin gpio){
	...
	uint8_t pin = gpio % NUMBER_OF_PINS_ON_EACH_PORT;
    GPIO_TypeDef *gpio_port = (GPIO_TypeDef *)(AHB2PERIPH_BASE + (0x0400UL << port));
    return (gpio_port->IDR & (0x01 << pin)) >> pin;
}
void gpio_calculate_port(GPIO_Pin gpio, uint8_t *pin, GPIO_TypeDef *port){
