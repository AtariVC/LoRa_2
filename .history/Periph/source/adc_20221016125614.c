...
void ADC_Init(){
	...
	RCC->CCIPR   |=  ( 3 << RCC_CCIPR_ADCSEL_Pos );  ower down mode
...
void ADC_Init_regular(int channel_num){
	...
	while ( ADC1->CR & ADC_CR_ADCAL ) {};
	if (channel_num <= 9) {
		ADC1->SMPR1 |= (7 << (channel_num * 3)); // set sample time to 2.5 cycles
	} else {
		ADC1->SMPR2 |= (7 << ((channel_num * 3) - 30));
	}
	ADC1->SMPR1 |= (7 << (6 * 3));
	ADC1->SMPR1 |= (7 << (7 * 3));
	ADC1->CFGR &= ~ADC_CFGR_CONT;	// single /run mode
	...
	// ADC1->JSQR |= (channel_num << 8) | (6 << 14) | (7 << 20) | (3 << 0);
	ADC1->JSQR |= (channel_num << 8) | (1 << 0);
