...
uint8_t OneWire_SendBit(OneWire *ow, uint8_t data){
    if(ow->uart->ISR & USART_ISR_ORE){
        ow->uart->ICR |= USART_ICR_ORECF;
        uint8_t buf = ow->uart->RDR;
    }
    if(ow->uart->ISR & USART_ISR_FE)
        ow->uart->ICR |= USART_ICR_FECF;
    if(ow->uart->ISR & USART_ISR_NE)
        ow->uart->ICR |= USART_ICR_NECF;
