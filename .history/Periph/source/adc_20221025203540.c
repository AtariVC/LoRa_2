...
void ADC_InitChannel(ADC *ADC_struct, uint8_t ch_num, GPIO_Pin gpio, ADC_CH_Group group, ADC_CH_SMP_Time smp_time){
    ...
    ADC_struct->channel[ADC_struct->__ch_counter].ch_num = ch_num;
    ADC_struct->channel[ADC_struct->__ch_counter].group = group;
    ADC_struct->channel[ADC_struct->__ch_counter].pin = gpio;
    ADC_struct->channel[ADC_struct->__ch_counter].smp_time = smp_time;
}
