...
void gpio_init(GPIO_Pin gpio, GPIO_Mode mode, GPIO_Config config, GPIO_Pull pull_up_down, GPIO_Speed speed){
    ...
    RCC->AHB2ENR |= 1 << (gpio_struct.port);
	gpio_struct.port->PUPDR &= ~(3 << (pin * 2));  // pull up / pull down register
	gpio_struct.port->PUPDR |= pull_up_down << (pin * 2);

	gpio_struct.port->OTYPER &= ~(1 << pin);
	gpio_struct.port->OTYPER |= (config << pin);

	gpio_struct.port->OSPEEDR &= ~(3 << (pin * 2));
	if(speed != Input) gpio_struct.port->OSPEEDR |= speed << (pin * 2);

	if(mode > 3){
		gpio_struct.port->MODER &= ~(3 << (pin * 2));
		gpio_struct.port->MODER |= 2 << (pin * 2);

		gpio_struct.port->AFR[pin/8] &= ~(0x0F << (pin * 4));
		pin/8 == 0 ? (gpio_struct.port->AFR[pin/8] |= ((mode-4) << (pin * 4))) : (gpio_port->AFR[pin/8] |= ((mode-4) << ((pin-8) * 4)));
	}
	else{
		gpio_struct.port->MODER &= ~(3 << (pin * 2));
		gpio_struct.port->MODER |= mode << (pin * 2);
	}
}
...
GPIO_Struct gpio_calculate(GPIO_Pin gpio){
	...
	uint8_t port = gpio / NUMBER_OF_PINS_ON_EACH_PORT;  // GPIOA = 1, GPIOB = 2, ..., GPIOE = 5
