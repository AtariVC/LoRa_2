...
uint32_t RTC_struct_brief_time_converter(RTC_struct_brief volatile *br_data){
...
uint32_t RTC_struct_brief_date_converter(RTC_struct_brief volatile *br_data){
...
void RTC_data_update(RTC_struct_brief volatile *br_data){
...
uint32_t EncodeDateTime(RTC_struct_brief *dt)
{
        uint8_t a = dt->months < 3; // а = 1, если месяц январь или февраль
        int16_t y = dt->years - a - 2000;  // y = отнимаем от года 1, если а = 1, а так же 2000;
