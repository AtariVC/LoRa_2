...
GPIO_Struct gpio_calculate(GPIO_Pin gpio){
    if(gpio == uninitialized)
        while(1);
    uint8_t gpio_pin = gpio - 1;
    GPIO_Struct temp = {0};
	temp.port = gpio_pin / NUMBER_OF_PINS_ON_EACH_PORT;  // GPIOA = 1, GPIOB = 2, ..., GPIOE = 5
	temp.pin = gpio_pin % NUMBER_OF_PINS_ON_EACH_PORT;  // PB0 = 0, PA1 = 1, PD4 = 4
    temp.port = (GPIO_TypeDef *)(AHB2PERIPH_BASE + (0x0400UL * temp.port) );
    return temp;
}
