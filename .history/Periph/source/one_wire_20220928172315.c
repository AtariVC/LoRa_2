...
int8_t hasNextRom(OneWire *ow, uint8_t *ROM) {
    ...
    while(rom_bit_counter < OneWire_ROM_ID_lenth){
        ...
        if (cB == cmp_cB && cB == 0) {  //коллизия
            if (rom_bit_counter == ow->last_diff_bit_position) searchDirection = 1;
            else {
                if (rom_bit_counter > ow->last_diff_bit_position) searchDirection = 0;
                else searchDirection = (uint8_t) ((ow->lastROM[byteNum] >> rom_bit_counter % 8) & 0x01);
                if (searchDirection == 0) zeroFork = rom_bit_counter;
            }
        }
        // сохраняем бит
        if (searchDirection)
