...
void ADC_Handler(){
    ...
    if(ADC1->ISR & ADC_ISR_EOC){  // Inside the regular sequence, after each conversion is complete
        ADC1->ISR |= ADC_ISR_EOC;
    }
    if(ADC1->ISR & ADC_ISR_EOS){  // After the regular sequence is complete
        ADC1->ISR |= ADC_ISR_EOS;
    }
    if(ADC1->ISR & ADC_ISR_JEOC){  // Inside the injected sequence, after each conversion is complete:
        ADC1->ISR |= ADC_ISR_JEOC;
    }
    if(ADC1->ISR & ADC_ISR_JEOS){  // After the injected sequence is complete
