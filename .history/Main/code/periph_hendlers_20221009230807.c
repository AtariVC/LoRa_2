...
//		while(!(USART3->ISR & USART_ISR_TC));
        ...
        array[rx_counter] = data;
        rx_counter++;
        FIFO_PUSH(sim7000g.rx_fifo, data);
        if(data == '\n'){
            if((strstr(array, "OK\r\n") != 0) || (strstr(array, "ERROR\r\n") != 0)){
            // if(array[rx_counter - 1] == '\r' && array[rx_counter - 2] == 'K' && array[rx_counter - 3] == 'O')
                sim7000g.waiting_answer_flag = 0;
                rx_counter = 0;
            }
        }
	}
    ...
    if(USART3->ISR & USART_ISR_FE){
        FIFO_PUSH(sim7000g.rx_fifo, USART3->RDR);
