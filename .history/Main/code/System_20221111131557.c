...
void System_Init(){
    ...
    for(uint8_t i = 0; i < 12; i++){
        OneWireStatus status = OneWire_MatchRom(&ow, (RomCode*)(&gs1_ow_nums));
        if(status == ONE_WIRE_OK){
            OneWire_Write(&ow, DS18B20_READ_SCRATCHPAD);
            OneWire_ReadArray(&ow, (uint8_t *)(&(sensor[i].scratchpad)), 9);
            sensor[i].temperature = (uint32_t)(sensor->scratchpad.temperature) * 0.0625;
        }
    }
