...
void MakeMeasure(){
    ...
    for(uint8_t i = 0; i < 11; i++){
        measured_data[i + 2] = adc.reg_channel_queue[i].result;
        measured_data[i + 14] = sensors[i].scratchpad.temperature;
    }
    ...
    if(buffer_ptr + sizeof(current_rtc) < sizeof(data_buffer)){
        ...
        buffer_ptr += sizeof(current_rtc) + sizeof(measured_data);
    }
    RTC_get_time(&current_rtc);

    memcpy(data_buffer + buffer_ptr, (uint8_t *)(&gs2_buffer), sizeof(gs2_buf_ptr));
    memcpy(data_buffer + buffer_ptr, (uint8_t *)(&gs3_buffer), sizeof(gs3_buf_ptr));
    buffer_ptr = 0;
    gs2_buf_ptr = 0;
    gs3_buf_ptr = 0;
}
int main(){
	...
	// delay_action(5000, 0, LED_BLINK);
		...
		delay_action(6000000, 1, SEND_TO_SERVER);  // every 5 min
        delay_action(1000, 1, MakeMeasure);
