        ...
        if(gprs_init_flag == 0){
            ...
            if(sim7000g.ip_status==GPRS_CONNECT_OK){
                ...
                GSM_SendTCP(&sim7000g, (char*)data_buffer, buffer_ptr);
                    SX126x_WaitTCPServerAcnoledge(&sim7000g, 2500);
                    SX126x_WaitTCPServerConnection(&sim7000g, 2500);
                    ...
                    GSM_OpenConnection(&sim7000g, "hirundo.ru", "8083");
                    timeout = 2500;
                    for(uint16_t timeout = 2500; !sim7000g.status.tcp_server_connected && timeout > 0; timeout--) Delay(1);
                    GSM_SendTCP(&sim7000g, (char*)data_buffer, buffer_ptr);
                    for(uint16_t timeout = 2500; !sim7000g.status.tcp_server_answer && timeout > 0; timeout--) Delay(1);
                }
                GSM_CloseConnections(&sim7000g);
            }
...
int main(){
	...
	// delay_action(5000, 0, LED_BLINK);
		...
		delay_action(10000, 1, SEND_TO_SERVER);
        delay_action(1000, 2, MakeMeasure);
