...
void SEND_TO_SERVER(){
    ...
    if(SX1268.self_addr == 1){
        ...
        if(buffer_ptr + gs2_buf_ptr < sizeof(data_buffer)){
            ...
            memcpy(data_buffer + buffer_ptr, (uint8_t *)(&data), 2);
            buffer_ptr += 2;
...
int main(){
	...
	// delay_action(5000, 0, LED_BLINK);
		...
		delay_action(1200000, 1, SEND_TO_SERVER);  // every 5 min
        delay_action(600000, 2, MakeMeasure);
