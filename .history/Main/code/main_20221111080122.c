...
void SEND_TO_SERVER(){
    ...
    if(SX1268.self_addr == 1){
        ...
        if(buffer_ptr + gs2_buf_ptr + gs3_buf_ptr + 4 < sizeof(data_buffer)){
            memcpy(data_buffer + buffer_ptr + gs2_buf_ptr + gs3_buf_ptr, (uint8_t *)(&gsm_vbat), 2);
            memcpy(data_buffer + buffer_ptr + gs2_buf_ptr + gs3_buf_ptr + 2, (uint8_t *)(&sim7000g.signal_level), 1);
        }
        ...
        if(sim7000g.ip_status==GPRS_CONNECT_OK){
            ...
            while(!sim7000g.status.tcp_server_answer && timeout--)
                Delay(1);
            if(!sim7000g.status.tcp_server_answer){
...
int main(){
	...
	// delay_action(5000, 0, LED_BLINK);
		...
		delay_action(20000, 1, SEND_TO_SERVER);  // every 5 min
        delay_action(10000, 2, MakeMeasure);
