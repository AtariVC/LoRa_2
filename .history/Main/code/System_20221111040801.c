...
void System_Init(){
    ...
    if(SX1268.self_addr != 1){
        ...
        for(uint8_t i = 0; i < 12; i++){
            memcpy(sensor_data.args, (uint8_t*)&adc.reg_channel_queue[i].result, 2);
            memcpy(sensor_data.args + 12, (uint8_t*)&sensors[i].scratchpad.temperature, 2);
        }
        memcpy(sensor_data.args + 12, (uint8_t*)&adc.vdda_mvolt, 2);
        gpio_state(TEMP_SENSOR_PWR_EN, LOW);
        gpio_state(SOIL_SENSOR_PWR_EN, LOW);
