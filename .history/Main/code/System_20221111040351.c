...
void System_Init(){
    ...
    if(SX1268.self_addr != 1){
        ...
        for(uint8_t i = 0; i < 48; i++){
            sensor_data.args[i] = adc.reg_channel_queue[i].result >> 8;
            sensor_data.args[i + 1] = adc.reg_channel_queue[i].result & 0xFF;
            sensor_data.args[i + 12] = sensors[i].scratchpad.temperature >> 8;
            sensor_data.args[i + 13] = sensors[i].scratchpad.temperature & 0xFF;
        }
        sensor_data.args[23] = adc.vdda_mvolt >> 8;
        sensor_data.args[24] = adc.vdda_mvolt & 0xFF;
        gpio_state(TEMP_SENSOR_PWR_EN, LOW);
        gpio_state(SOIL_SENSOR_PWR_EN, LOW);
