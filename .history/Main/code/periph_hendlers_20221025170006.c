...
void ADC1_IRQHandler(void){
    ...
    if(ADC1->ISR & ADC_ISR_EOC){  // regular channel end of conversion
        ADC1->ISR |= ADC_ISR_EOC;
    }
    if(ADC1->ISR & ADC_ISR_EOS){  // regular channel end of sequence
        ADC1->ISR |= ADC_ISR_EOS;
    }
    if(ADC1->ISR & ADC_ISR_JEOC){  // injective channel end of conversion
        ADC1->ISR |= ADC_ISR_JEOC;
    }
    if(ADC1->ISR & ADC_ISR_JEOS){  // injective channel end of sequence
