...
uint64_t reverse_bytes(uint64_t num){
    return (((num & 0xFF00000000000000) >> 56) | ((num & 0x00FF000000000000) >> 48) | ((num & 0x0000FF0000000000) >> 40) | ((num & 0x000000FF00000000) >> 32) | ((num & 0x00000000FF000000) >> 24) | ((num & 0x0000000000FF0000) >> 8) | ((num & 0x000000000000FF00) << 8) | ((num & 0x00000000000000FF) << 56) );
}
void System_Init(){
    ...
    if(DS18B20_Init(sensors, &ow)){
        ...
        for(uint8_t i = 0; i < 12; i++){
            ...
            ser_nums[i] = reverse_bytes(*(uint64_t *)(sensors[i].serialNumber));
        }
    }
    while(1){
        Delay(500);
